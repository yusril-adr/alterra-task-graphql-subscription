...
import Mutation from './services/apollo/mutation';
import Subscription from './services/apollo/subscription';
...
const TodoList = () => {
  ...
  const { loading: fetchLoading, error: fetchError, data } = useSubscription(Subscription.subsTodoList);

  const [addTodo, { error: addTodoError, loading: addTodoLoading }] = useMutation(Mutation.addTodo);

  const [updateTodoCompletedStatus, { error: updateTodoError, loading: updateTodoLoading }] = useMutation(Mutation.updateTodoCompletedStatus, {
    refetchQueries: [
      {
  ...
  const [deleteTodoByid, { error: deleteTodoError, loading: deleteTodoLoading }] = useMutation(Mutation.deleteTodoById, {
    refetchQueries: [
      {
        query: Query.getTodoById,
        variables: { id: currentShownId },
  ...
  if (fetchError || addTodoError || updateTodoError || deleteTodoError || searchError) {
    alert(fetchError.message || addTodoError.message || updateTodoError.message || deleteTodoError.message || searchError.message) 
  }
    ...
    <>
      <div className='container'>
        ...
        <ul className='todo-list js-todo-list'>
          {data?.list.map(({ id, title, completed }) => (
            <Todo
              key={id}
              id={id}
